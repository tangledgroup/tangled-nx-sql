"""Tests for nx.relabel_nodes and nx.convert_node_labels_to_integers with nx_sql graphs.

Ported from NetworkX test suite (test_relabel.py). Tests verify that the top-level
NX relabel functions work correctly with Graph/DiGraph/MultiGraph/MultiDiGraph backends.

Skipped tests expose known nx_sql implementation gaps:
- MultiGraph/MultiDiGraph missing edges(keys=True) support
- String edge keys not supported (only int keys)
- DiGraph copy=False relabel hits DB dict issues on remove_node
"""
...
class TestRelabel:
    ...
    def test_relabel_nodes_graph(self, Graph):
        G = Graph("relabel_graph")
        G.add_edges_from([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")])
        mapping = {"A": "aardvark", "B": "bear", "C": "cat", "D": "dog"}
        H = nx.relabel_nodes(G, mapping)
        assert nodes_equal(H.nodes(), ["aardvark", "bear", "cat", "dog"])

    def test_relabel_nodes_orderedgraph(self, Graph):
        G = Graph("ordered_graph")
    ...
    @pytest.mark.skip(reason="nx_sql DiGraph copy=False relabel hits DB dict issue on remove_node (existing gap)")
    def test_relabel_nodes_digraph(self, DiGraph):
        G = DiGraph("relabel_digraph")
        G.add_edges_from([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")])
        mapping = {"A": "aardvark", "B": "bear", "C": "cat", "D": "dog"}
        H = nx.relabel_nodes(G, mapping, copy=False)
        assert nodes_equal(H.nodes(), ["aardvark", "bear", "cat", "dog"])

    @pytest.mark.skip(reason="nx_sql MultiGraph edges(keys=True) not supported (existing gap)")
    def test_relabel_nodes_multigraph(self, MultiGraph):
        G = MultiGraph("relabel_mg")
        G.add_edges_from([("a", "b"), ("a", "b")])
    ...
    @pytest.mark.skip(reason="nx_sql MultiDiGraph edges(keys=True) not supported (existing gap)")
    def test_relabel_nodes_multidigraph(self, MultiDiGraph):
        G = MultiDiGraph("relabel_mdg")
        G.add_edges_from([("a", "b"), ("a", "b")])
        mapping = {"a": "aardvark", "b": "bear"}
        G = nx.relabel_nodes(G, mapping, copy=False)
        assert nodes_equal(G.nodes(), ["aardvark", "bear"])
    ...
    def test_relabel_isolated_nodes_to_same(self, Graph):
        G = Graph("isolated")
    ...
    def test_relabel_nodes_missing(self, Graph):
        G = Graph("missing_map")
        G.add_edges_from([("A", "B"), ("A", "C"), ("B", "C"), ("C", "D")])
        mapping = {0: "aardvark"}
        # copy=True — unmapped nodes keep original labels
        H = nx.relabel_nodes(G, mapping, copy=True)
        assert nodes_equal(H.nodes, G.nodes)
        # copy=False — original graph unchanged if no keys match
        GG = G.copy()
        nx.relabel_nodes(G, mapping, copy=False)
        assert nodes_equal(G.nodes, GG.nodes)

    def test_relabel_copy_name(self, Graph):
        G = Graph("name_test")
    ...
    def test_relabel_toposort(self, Graph):
    ...
    @pytest.mark.skip(reason="nx_sql DiGraph copy=False relabel hits DB dict issue on remove_node (existing gap)")
    def test_relabel_selfloop(self, DiGraph, MultiDiGraph):
        G = DiGraph("selfloop_dg")
        G.add_edges_from([(1, 1), (1, 2), (2, 3)])
        G = nx.relabel_nodes(G, {1: "One", 2: "Two", 3: "Three"}, copy=False)
        assert nodes_equal(G.nodes(), ["One", "Three", "Two"])
        G = MultiDiGraph("selfloop_mdg")
        G.add_edges_from([(1, 1), (1, 2), (2, 3)])
        G = nx.relabel_nodes(G, {1: "One", 2: "Two", 3: "Three"}, copy=False)
        assert nodes_equal(G.nodes(), ["One", "Three", "Two"])
        G = MultiDiGraph("selfloop_mdg2")
        G.add_edge(1, 1)
        G = nx.relabel_nodes(G, {1: 0}, copy=False)
        assert nodes_equal(G.nodes(), [0])

    @pytest.mark.skip(reason="nx_sql string edge keys not supported + edges(keys=True) missing (existing gaps)")
    def test_relabel_multidigraph_inout_merge_nodes(self, MultiGraph, MultiDiGraph):
        for MG in (MultiGraph, MultiDiGraph):
            for cc in (True, False):
                G = MG("merge_mdg")
                G.add_edges_from([(0, 4), (1, 4), (4, 2), (4, 3)])
                G[0][4][0]["value"] = "a"
                G[1][4][0]["value"] = "b"
                G[4][2][0]["value"] = "c"
                G[4][3][0]["value"] = "d"
    ...
    @pytest.mark.skip(reason="nx_sql edges(keys=True) not supported (existing gap)")
    def test_relabel_multigraph_merge_inplace(self, MultiGraph):
        G = MultiGraph("merge_mg_inplace")
        G.add_edges_from([(0, 1), (0, 2), (0, 3), (0, 1), (0, 2), (0, 3)])
        G[0][1][0]["value"] = "a"
        G[0][2][0]["value"] = "b"
        G[0][3][0]["value"] = "c"
        mapping = {1: 4, 2: 4, 3: 4}
    ...
    @pytest.mark.skip(reason="nx_sql edges(keys=True) not supported (existing gap)")
    def test_relabel_multidigraph_merge_inplace(self, MultiDiGraph):
        G = MultiDiGraph("merge_mdg_inplace")
    ...
    @pytest.mark.skip(reason="nx_sql string edge keys not supported + edges(keys=True) missing (existing gaps)")
    def test_relabel_multidigraph_inout_copy(self, MultiDiGraph):
        G = MultiDiGraph("merge_mdg_copy")
        G.add_edges_from([(0, 4), (1, 4), (4, 2), (4, 3)])
        G[0][4][0]["value"] = "a"
        G[1][4][0]["value"] = "b"
        G[4][2][0]["value"] = "c"
    ...
    @pytest.mark.skip(reason="nx_sql edges(keys=True) not supported (existing gap)")
    def test_relabel_multigraph_merge_copy(self, MultiGraph):
        G = MultiGraph("merge_mg_copy")
        G.add_edges_from([(0, 1), (0, 2), (0, 3)])
        G[0][1][0]["value"] = "a"
        G[0][2][0]["value"] = "b"
        G[0][3][0]["value"] = "c"
    ...
    @pytest.mark.skip(reason="nx_sql edges(keys=True) not supported (existing gap)")
    def test_relabel_multidigraph_merge_copy(self, MultiDiGraph):
        G = MultiDiGraph("merge_mdg_copy2")
        G.add_edges_from([(0, 1), (0, 2), (0, 3)])
        G[0][1][0]["value"] = "a"
        G[0][2][0]["value"] = "b"
        G[0][3][0]["value"] = "c"
    ...
    @pytest.mark.skip(reason="nx_sql string edge keys not supported (existing gap)")
    def test_relabel_multigraph_nonnumeric_key(self, MultiGraph, MultiDiGraph):
        for MG in (MultiGraph, MultiDiGraph):
            for cc in (True, False):
                G = MG("nonnumeric_key")
