...
This example embeds the GML data inline so it runs without external files.

The data represents American football games between Division IA colleges
...
# Conference mapping from the original data
CONFERENCE_NAMES = {
    0: "Atlantic Coast",
    1: "Big East",
    2: "Big Ten",
...
# Embedded GML data (football network from M. Newman)
FOOTBALL_GML = """\
...
@print_docstring
def demo_football():
    """Load the football network and compute network statistics."""

    with Session() as session:
        # Parse GML data
        gml_lines = FOOTBALL_GML.split("\n")[1:]  # skip Creator line
        G_nx = nx.parse_gml(gml_lines)

        G = nx_sql.Graph(session, name="football_demo")
        for node in G_nx.nodes():
            G.add_node(node, **G_nx.nodes[node])
        ...
        # Print degree for each team
        print("\nTeam degrees (games played):")
        for node in sorted(G.nodes()):
            label = G.nodes[node].get("label", str(node))
            degree = G.degree(node)
            conf_id = G.nodes[node].get("value", -1)
            conf_name = CONFERENCE_NAMES.get(conf_id, f"unknown({conf_id})")
            bar = "#" * degree
            print(f"  {label:25s} games={degree:2d}  {bar}  [{conf_name}]")
        ...
        # Count teams per conference in each community
        for i, comm in enumerate(sorted_comms):
            conf_counts = {}
            for node in comm:
                c = G.nodes[node].get("value", -1)
                conf_counts[c] = conf_counts.get(c, 0) + 1
            dominant_conf = max(conf_counts.items(), key=lambda x: x[1])
            conf_name = CONFERENCE_NAMES.get(dominant_conf[0], f"unknown({dominant_conf[0]})")
            print(f"\nCommunity {i}: {len(comm)} teams (dominant: {conf_name})")
            for conf_id, count in sorted(conf_counts.items(), key=lambda x: -x[1]):
                cname = CONFERENCE_NAMES.get(conf_id, f"unknown({conf_id})")
                print(f"  {cname}: {count} teams")
    ...
    with Session() as session:
        ...
        # Verify a few teams by label lookup
        team_names = [n for n in G2.nodes() if 'Team' in str(n) or n in ['BrighamYoung', 'LouisianaState', 'Hawaii']]
        for node in sorted(team_names)[:3]:
            label = G2.nodes[node].get("label", node)
            print(f"  Node: {label}")
