...
letter, resulting in 14,135 edges. This example is described in Section 1.1 of
    ...
    Donald E. Knuth, "The StanfordGraphBase: A Platform for Combinatorial
    Computing", ACM Press, New York, 1993.
    http://www-cs-faculty.stanford.edu/~knuth/sgb.html

This example embeds the data inline so it runs without external files.
"""
...
WORDS_DATA = """\
* This file words_dat.txt is part of NetworkX and is distributed 
...
def load_words():
    """Load the word list from embedded data."""
    words = set()
    for line in WORDS_DATA.split("\n"):
        if line.startswith("*"):
            continue
        w = str(line[0:5])
...
@print_docstring
def demo_words():
    """Build the word ladder graph from 5-letter English words."""
    with Session() as session:
        words = load_words()
        print(f"Loaded {len(words)} five-letter English words.")

        G_nx = generate_graph(words)
        print(f"Two words are connected if they differ in one letter.")
        print(f"Graph: {G_nx.number_of_nodes()} nodes, {G_nx.number_of_edges()} edges")

        G = nx_sql.Graph(session, name="words_demo")
        ...
        if n_components > 1:
            ...
            print("Single connected component — all words form one ladder!")
        ...
        print(f"\n=== Top 10 Words by Connections ===")
        top = sorted(G.degree(), key=lambda x: x[1], reverse=True)[:10]
        for node, degree in top:
            bar = "#" * min(degree, 50)
            print(f"  {node}: {degree} connections {bar}")

        # Shortest paths between example pairs
        print(f"\n=== Word Ladder Paths ===")
        for source, target in [("chaos", "order"), ("nodes", "graph"), ("pound", "marks")]:
            try:
                shortest_path = nx.shortest_path(G, source, target)
                print(f"{source} -> {target}: {len(shortest_path)-1} steps")
                for i, n in enumerate(shortest_path):
                    marker = " <--" if i == 0 or i == len(shortest_path)-1 else ""
                    print(f"  {i}. {n}{marker}")
            except nx.NetworkXNoPath:
                print(f"{source} -> {target}: None (no path)")

        # Show a few word ladder chains
        print(f"\n=== Sample Word Ladders ===")
        test_pairs = [("black", "white"), ("stone", "round"), ("great", "least")]
        for src, tgt in test_pairs:
            try:
                path = nx.shortest_path(G, src, tgt)
                joined = ' -> '.join(path)
                print(f"  {src} -> {joined} ({len(path)-1} steps)")
            except nx.NetworkXNoPath:
    ...
    with Session() as session:
        gmodel = session.execute(
            nx_sql.select(GraphModel).where(GraphModel.name == "words_demo")
        ).scalar_one()
