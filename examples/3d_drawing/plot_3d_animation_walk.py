...
@print_docstring
def demo_3d_animation_walk():
    """Random walk on a rotating 3D graph saved as GIF."""

    with Session() as session:
        G_nx = nx.dodecahedral_graph()
        G = nx_sql.Graph(session, name="3d_animation_walk_demo")
        ...
        def init():
            ...
            ax.plot(*nodes[node], alpha=1, marker="s", color="red")
            ax.grid(False)
            ax.set_axis_off()
        ...
        ani.save("examples/3d_drawing/plot_3d_animation_walk_output.gif",
                 writer="pillow", fps=20)
        plt.close()
        print("Animation saved to examples/3d_drawing/plot_3d_animation_walk_output.gif")

        session.commit()


if __name__ == "__main__":
    demo_3d_animation_walk()
